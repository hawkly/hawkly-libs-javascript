 ...
 */
export class Forge {
  private wardenKeySetDirectory: string;
  private guardKeySetDirectory: string;

  private maxKeySetsValid: number = 4;

  private maxKeySetValidDays: number = 8;
  ...
  public async createNewKeySetSet(expiry: number): Promise<ForgeKeySet> {
    ...
    const rawKeys: ForgeKeySet[] = await Promise.all(
      [...Array(this.maxKeySetsValid)].map(
        async (_, i: number) => {
          ...
          return await this.createNewKeySetSet(expires);
        },
      ));
